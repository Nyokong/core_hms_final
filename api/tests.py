...
class CustomSignupSerializerTests(TestCase):
    ...
    def add_session_to_request(self, request):
        """Attach a session to the request without using middleware."""
        request.session = self.client.session

    def test_valid_data_creates_user_with_session(self):
        valid_data = {
            'username': 'testuser',
            'email': 'testuser@example.com',
        ...
        request = self.client.post('/api/signup/', valid_data)
        self.add_session_to_request(request)
        serializer = CustomSignupSerializer(data=valid_data, context={'request': request})
        self.assertTrue(serializer.is_valid(), serializer.errors)
